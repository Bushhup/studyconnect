      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-64 hidden md:block">
            ...
            <Input placeholder="Filter alerts..." className="pl-10 h-10 rounded-full border-none bg-card shadow-sm text-xs" />
          </div>
          <Button variant="outline" className="rounded-full gap-2 border-border bg-card shadow-sm h-10 px-6 font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="space-y-6">
          <Card className="border-none shadow-sm bg-slate-900 text-white rounded-[2rem] p-6 relative overflow-hidden">
            <Bell className="absolute right-[-10px] bottom-[-10px] h-20 w-20 text-white/5 -rotate-12" />
          ...
          <div className="space-y-1">
            {[
              ...
              <Button key={cat.label} variant="ghost" className="w-full justify-start gap-3 h-11 rounded-xl text-xs font-bold text-muted-foreground hover:bg-card hover:shadow-sm group">
                <cat.icon className="h-4 w-4 text-muted-foreground group-hover:text-primary transition-colors" /> {cat.label}
              </Button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          {ANNOUNCEMENTS.map((item) => (
            <Card key={item.id} className={cn(
              "border-none shadow-sm rounded-2xl overflow-hidden transition-all hover:shadow-md bg-card",
              ...
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    <Badge className={cn(
                      "text-[8px] font-bold uppercase border-none px-2 h-5 flex items-center",
                      item.important ? "bg-primary text-white" : "bg-muted text-muted-foreground"
                    )}>
                      {item.category}
                    </Badge>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed font-body">{item.content}</p>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-border/50 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-foreground uppercase tracking-tight">{item.author}</p>
                      <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-tighter">Verified Official</p>
                    </div>
                  </div>
                  <Button variant="ghost" size="sm" className="text-[10px] font-bold text-primary uppercase h-8 px-4 rounded-lg hover:bg-primary/5">
