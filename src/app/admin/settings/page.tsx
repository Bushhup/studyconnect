...
import { 
  ...
  ArrowRight, Loader2, DatabaseBackup
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      toast({ title: 'Database Synchronized', description: 'Institutional records have been successfully seeded.' });
    } catch (e: any) {
                ...
                <Card className="border-none shadow-sm bg-card rounded-[2rem] overflow-hidden">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-primary/10 rounded-xl"><Lock className="h-5 w-5 text-primary" /></div>
                      <div>
                  ...
                  <CardContent className="space-y-6">
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/30 border border-transparent hover:border-border transition-all">
                      <div className="space-y-0.5">
                        <Label className="text-sm font-bold flex items-center gap-2"><Smartphone className="h-4 w-4 text-primary" /> 2FA Authorization</Label>
                        <p className="text-xs text-muted-foreground">Require mobile verification for administrative modifications.</p>
                      </div>
                      <Switch defaultChecked />
                    </div>
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/30 border border-transparent hover:border-border transition-all">
                      <div className="space-y-0.5">
                        <Label className="text-sm font-bold flex items-center gap-2"><Globe className="h-4 w-4 text-primary" /> Public Student Directory</Label>
                        <p className="text-xs text-muted-foreground">Allow searching student records via public landing page.</p>
                      </div>
                      <Switch />
                    </div>
                  </CardContent>
                  <CardContent className="pt-0 border-t border-dashed mt-4 flex justify-end">
                    <Button onClick={handleSave} className="rounded-xl h-11 px-8 mt-4 font-bold shadow-lg shadow-primary/20">Save Security Rules</Button>
                  </CardContent>
                </Card>
              </motion.div>
            )}
                  ...
                  <CardContent>
                    <Tabs defaultValue="background" className="w-full">
                      <TabsList className="grid w-full grid-cols-4 h-12 bg-muted p-1 rounded-xl mb-8">
                        <TabsTrigger value="background" className="gap-2 rounded-lg text-xs font-bold"><Monitor className="h-3.5 w-3.5" /> Workspace</TabsTrigger>
                        <TabsTrigger value="common" className="gap-2 rounded-lg text-xs font-bold"><Type className="h-3.5 w-3.5" /> Text</TabsTrigger>
                        <TabsTrigger value="special" className="gap-2 rounded-lg text-xs font-bold"><Layout className="h-3.5 w-3.5" /> Accents</TabsTrigger>
                        <TabsTrigger value="navigation" className="gap-2 rounded-lg text-xs font-bold"><GripVertical className="h-3.5 w-3.5" /> Nav Hub</TabsTrigger>
                      </TabsList>
                      <TabsContent value="background" className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        {bgThemes.map((t) => (
                          <button key={t.id} onClick={() => setBg(t.id)} className={cn("group relative flex flex-col items-center gap-3 p-6 rounded-[1.5rem] border-2 transition-all duration-300", theme.bg === t.id ? "border-primary bg-primary/5 shadow-md" : "border-border hover:border-primary/20 bg-muted/30")}>
                            <div className={cn("h-12 w-12 rounded-full shadow-inner", t.color)} />
                            ...
                            {theme.bg === t.id && <div className="absolute top-2 right-2 bg-primary text-white p-1 rounded-full shadow-lg"><Check className="h-3 w-3" /></div>}
                          </button>
                        ))}
                      </TabsContent>
                      <TabsContent value="common" className="space-y-3">
                        {textThemes.map((t) => (
                          <button key={t.id} onClick={() => setText(t.id)} className={cn("w-full flex items-center justify-between p-5 rounded-2xl border-2 transition-all", theme.text === t.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/20 bg-muted/30")}>
                            <div className="text-left"><p className={cn("text-base font-bold", theme.text === t.id ? "text-primary" : "text-foreground")}>{t.name}</p><p className="text-xs text-muted-foreground font-body">{t.desc}</p></div>
                            {theme.text === t.id && <Check className="h-5 w-5 text-primary" />}
                          </button>
                        ))}
                      </TabsContent>
                      <TabsContent value="special" className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                        {primaryThemes.map((t) => (
                          <button key={t.id} onClick={() => setPrimary(t.id)} className={cn("group relative flex flex-col items-center gap-3 p-5 rounded-2xl border-2 transition-all", theme.primary === t.id ? "border-primary bg-primary/5 shadow-md" : "border-border hover:border-primary/20 bg-muted/30")}>
                            <div className={cn("h-12 w-12 rounded-xl shadow-lg", t.color)} />
                            ...
                            {theme.primary === t.id && <div className="absolute top-2 right-2 bg-primary text-white p-1 rounded-full shadow-lg"><Check className="h-3 w-3" /></div>}
                          </button>
                        ))}
                      </TabsContent>
                      <TabsContent value="navigation" className="space-y-4">
                        {navStyles.map((s) => (
                          <button key={s.id} onClick={() => setNavStyle(s.id)} className={cn("w-full flex items-center gap-5 p-5 rounded-2xl border-2 transition-all text-left", theme.navStyle === s.id ? "border-primary bg-primary/5 shadow-md" : "border-border hover:border-primary/20 bg-muted/30")}>
                            <div className={cn("p-4 rounded-2xl shadow-sm", theme.navStyle === s.id ? "bg-primary text-white" : "bg-card text-muted-foreground")}><s.icon className="h-6 w-6" /></div>
                            <div className="flex-1"><p className={cn("text-base font-bold", theme.navStyle === s.id ? "text-primary" : "text-foreground")}>{s.name}</p><p className="text-xs text-muted-foreground font-body">{s.desc}</p></div>
            ...
            {activeSection === 'notifications' && (
              <motion.div key="notifications" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                <Card className="border-none shadow-sm bg-card rounded-[2rem] overflow-hidden">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-primary/10 rounded-xl"><Bell className="h-5 w-5 text-primary" /></div>
                      <div>
                  ...
                  <CardContent className="space-y-6">
                    <div className="grid gap-4">
                      {[{ label: 'Global System Alerts', icon: AlertCircle, desc: 'Critical system-wide updates.' }, { label: 'Academic Reminders', icon: Calendar, desc: 'Automatic alerts for deadlines.' }].map((rule) => (
                        <div key={rule.label} className="flex items-center justify-between p-5 rounded-2xl bg-muted/30 border border-transparent hover:border-border transition-all group">
                          <div className="flex items-start gap-4">
                            <div className="p-2.5 bg-card rounded-xl shadow-sm group-hover:scale-110 transition-transform"><rule.icon className="h-4 w-4 text-primary" /></div>
                            <div className="space-y-0.5"><Label className="text-sm font-bold">{rule.label}</Label><p className="text-xs text-muted-foreground font-body">{rule.desc}</p></div>
                          </div>
                          <Switch defaultChecked />
                        </div>
                      ))}
                  ...
                  <CardContent className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="p-6 rounded-3xl bg-emerald-50 border border-emerald-100 space-y-4">
                         ...
                         <p className="text-xs text-emerald-700/80 leading-relaxed font-body">Institutional identity nodes and hierarchical records are synchronized with 99.9% uptime.</p>
                         <Button variant="outline" className="w-full bg-white/50 border-emerald-200 text-emerald-700 font-bold gap-2 hover:bg-white rounded-xl"><RefreshCcw className="h-3.5 w-3.5" /> Force Metadata Sync</Button>
                      </div>
                      ...
                      {isMasterAdmin && (
                        <div className="p-6 rounded-3xl bg-blue-50 border border-blue-100 space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <p className="font-bold text-blue-900">System Recovery</p>
                          </div>
                          <p className="text-xs text-blue-700/80 leading-relaxed font-body">Initialize or reset the institutional database with standard academic seed records.</p>
                          <Button 
                            variant="outline" 
                            className="w-full bg-white/50 border-blue-200 text-blue-700 font-bold gap-2 hover:bg-white rounded-xl"
                            onClick={handleSeed}
                            disabled={isSeeding}
                          >
                            {isSeeding ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCcw className="h-3.5 w-3.5" />}
                    ...
                    <div className="pt-6 border-t border-dashed">
                      ...
                      <Button variant="outline" className="border-destructive/20 text-destructive hover:bg-red-50 rounded-xl h-12 font-bold uppercase text-[10px] tracking-widest gap-2"><ShieldCheck className="h-4 w-4" /> Reset Identity Keys</Button>
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
