  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-headline font-bold text-foreground tracking-tight">Record Attendance</h1>
          <p className="text-muted-foreground mt-1">Mark student presence for your current academic session.</p>
        </div>
        <Button onClick={handleSave} className="rounded-full shadow-lg shadow-primary/20 gap-2 h-11 px-8 font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="space-y-1.5">
          ...
          <Select value={degree} onValueChange={setDegree}>
            <SelectTrigger className="bg-card border-none shadow-sm rounded-xl h-11">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
             <div className="relative w-full sm:w-80">
                ...
                <Input 
                  ...
                  className="pl-10 bg-muted/50 border-none h-11 rounded-xl shadow-none"
                  value={search}
             ...
             <div className="flex items-center gap-4">
                ...
                <Button onClick={handleMarkAllPresent} variant="outline" size="sm" className="rounded-full h-8 text-[10px] font-bold uppercase bg-card">Mark All Present</Button>
             </div>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          <div className="divide-y divide-border">
            {attendance.map((student) => (
              <div key={student.id} className="p-5 flex items-center justify-between hover:bg-muted/20 transition-colors group">
                <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  {[
                    { val: 'present', label: 'Present', icon: CheckCircle2, activeClass: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' },
                    { val: 'absent', label: 'Absent', icon: XCircle, activeClass: 'bg-destructive/10 text-destructive border-destructive/20' },
                    { val: 'late', label: 'Late', icon: Clock, activeClass: 'bg-amber-500/10 text-amber-600 border-amber-500/20' },
                  ].map((btn) => (
                    <Button
                      key={btn.val}
                      variant="ghost"
                      ...
                      className={cn(
                        ...
                        student.status === btn.val 
                          ? btn.activeClass 
                          : "border-border text-muted-foreground bg-transparent hover:bg-muted/30"
                      )}
                    >
